...
namespace HOT.OracleDAL
{
    ...
    public class SysManage1 : ISysManage//这里必须实现接口，否则工厂类创建接口报错
	{
        ...
        public int AddTreeNode(HOT.Model.SysNode model)
		{
        ...
        public void UpdateNode(HOT.Model.SysNode model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// <returns></returns>
        public HOT.Model.SysNode GetNode(int NodeID)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select * from S_Tree ");	
			strSql.Append(" where NodeID=@NodeID");
            ...
            HOT.Model.SysNode node = new HOT.Model.SysNode();
