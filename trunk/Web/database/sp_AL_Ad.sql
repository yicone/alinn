...
@Img nvarchar(100)
 ...
 AS 
	...
	INSERT INTO AL_Ad(
	[AdId],[AdGroupId],[Title],[Content],[Url],[UrlText],[SizeId],[Img]
	)VALUES(
	@AdId,@AdGroupId,@Title,@Content,@Url,@UrlText,@SizeId,@Img
	)
...
@Img nvarchar(100)
 AS 
	UPDATE AL_Ad SET 
	[AdGroupId] = @AdGroupId,[Title] = @Title,[Content] = @Content,[Url] = @Url,[UrlText] = @UrlText,[SizeId]=@SizeId,[Img] = @Img
	WHERE AdId=@AdId 
...
@AdId uniqueidentifier
 AS 
	SELECT 
	AdId,AdGroupId,Title,Content,Url,UrlText,SizeId,AuditState,Img
	 FROM AL_Ad
	 WHERE AdId=@AdId 
...
CREATE PROCEDURE UP_AL_Ad_GetList
 AS 
	SELECT 
	AdId,AdGroupId,Title,Content,Url,UrlText,AuditState,Img
	 FROM AL_Ad
...
set @sql='SELECT AdId,AdGroupId,Title,Content,Url,UrlText,SizeId,AuditState,Img FROM AL_Ad WHERE AdGroupId='''+@strWhere +''''
exec(@sql)
GO